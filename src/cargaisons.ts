...
export function cargaisonsTraitement() {
  ...
  const listeCargaisonContent = document.getElementById('listeCargaisonContent') as HTMLElement;
  const imageCargaisonInput = document.querySelector('.image-cargaison-input') as HTMLInputElement;
  const imageCargaison = document.getElementById('image-cargaison') as HTMLImageElement;
  const uneCargaisonElement = document.querySelectorAll('#uneCargaisonElement') as NodeListOf<HTMLElement>;

  let listeCargaisons: ICargaison[] = [];



  /** Déclaration des fonctions**/

  const produitAccepter = (cargaison: ICargaison): string[] => {
  ...
  const pourcentageEspaceOccupe = (cargaison: ICargaison) : number => {
    // let c : Cargaison ;
    if (cargaison.typec == 1){
      let c = new Routiere();
  ...
  const templateListCargaison = (cargaison: ICargaison) => {
    return `
  ...
  const buildListCargaison = () => {
    ...
    listeCargaisons.forEach((cargaison) => {
      listeCargaisonContent.insertAdjacentHTML(
        "beforeend",
        templateListCargaison(cargaison)
      );
    });
  };
...
// YYYY/MM/DD format to 2022-02-01T16:45:30
  function formatDate2(originalDate: string) {
...
// 2024-05-02 to 02/05/2024
  function formatDate3(originalDate: string) {
...
// 02/05/2024 to 2024-05-02
  function formatDate4(originalDate: string) {
    // Split the original date string by "/"
    let parts = originalDate.split("/");
...
// 2022-02-01T16:45:30 to 2024-05-02
  function formatDate5(originalDate: string) {
    let dateObj = new Date(originalDate);
...
// YYYY-MM-DD format to 2022-02-01T16:45:30
  function formatDate6(originalDate: string, heureChoisit: string) {
    let parts = originalDate.split("-");
  ...
  function removeListElement() {
    let uneCargaisonElement = document.querySelectorAll('#uneCargaisonElement') as NodeListOf<HTMLElement>;
  ...
  imageCargaisonInput.addEventListener("input", (e: Event) => {
    imageCargaison.src = imageCargaisonInput.value;
  })

  addCargaisonForm.addEventListener("submit", (e: Event) => {
    e.preventDefault();
